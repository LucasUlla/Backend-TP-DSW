...
export async function getAllFees(clientId, period) {
    const em = getEm();
    const where = {};
    if (clientId)
...
export async function getOneFee(id) {
    const em = getEm();
...
async function calculateCoursesAmount(clientId) {
    const em = getEm();
    const inscriptions = await em.find(Inscription, { client: clientId }, { populate: ['course', 'course.sport'] });
    let coursesAmount = 0;
    for (const insc of inscriptions) {
        const lastPrice = await em.findOne(Price, { sport: insc.course.sport.id }, { orderBy: { id: 'desc' } } // el precio más reciente cargado para ese sport o tambien puede ser date
        );
...
export async function generateFeeForClient(clientId, period) {
    const em = getEm();
...
export async function generateFeesForAllClients(period) {
    const em = getEm();
...
////////////////////////////////////////////////////////////////////////////////////////
//# sourceMappingURL=fee.services.js.map